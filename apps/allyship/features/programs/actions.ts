...
// Backfill program_controls for a program
export async function backfillProgramControls(programId: string) {
  const supabase = await createClient()
  ...
  try {
    ...
    const { data: frameworkControls, error: controlsError } = await supabase
      .from("framework_controls")
      .select("control_id")
      .eq("framework_id", program.framework_id)
    ...
    if (!frameworkControls || frameworkControls.length === 0) {
      return {
        success: false,
        error: {
          message: `No controls found for framework ${program.framework_id}`,
    ...
    // Check if program_controls already exist
    ...
    const existingControlIds = new Set(
      existingControls?.map((c) => c.control_id) || []
    )

    // Only create controls that don't exist yet
    const newControls = frameworkControls
      .filter((fc) => !existingControlIds.has(fc.control_id))
      .map((fc) => ({
    ...
    if (newControls.length > 0) {
      const { error: programControlsError } = await supabase
        .from("program_controls")
        .insert(newControls)

      if (programControlsError) {
        console.error("Program controls creation error:", programControlsError)
        return {
          success: false,
          error: {
            message: "Failed to create program controls",
            code: "create_program_controls_failed",
            status: 500,
          },
        }
      }
    ...
    revalidatePath(`/spaces/${program.account_id}/programs/${programId}`)

    return {
      success: true,
      data: {
        controls_added: newControls.length,
        total_controls: frameworkControls.length,
      },
    }
  } catch (error) {
    return {
      success: false,
      error: {
        message: "Failed to backfill program controls",
