...
export class ColorContrastTool extends BaseTool {
  ...
  private getEffectiveColor(el: HTMLElement): string {
    let colorObj: Colord = colord(window.getComputedStyle(el).color)
  ...
  private getBackgroundColors(el: HTMLElement): string[] {
    ...
    while (currentElement && currentElement !== document.body) {
      ...
      if (opacity < 1 && colors.length > 0) {
        ...
        if (lastColor) {
          colors[colors.length - 1] = colord(lastColor)
            .alpha(opacity)
            .toRgbString()
        }
      }
...
export const checkColorContrast = (
  ...
  const result = colorContrastTool.run(mode)
  if (!result) {
