...
-- Create function with single JSON parameter
CREATE OR REPLACE FUNCTION public.create_scan_with_website_and_page(params JSONB)
RETURNS TABLE (
  id UUID,
  status TEXT,
...
DECLARE
  ...
  -- Extract parameters
  p_website_url TEXT = params->>'p_website_url';
  p_page_url TEXT = params->>'p_page_url';
  p_page_path TEXT = params->>'p_page_path';
  p_space_id UUID = (params->>'p_space_id')::UUID;
  p_user_id UUID = (params->>'p_user_id')::UUID;
BEGIN
  -- Validate parameters
...
GRANT EXECUTE ON FUNCTION public.create_scan_with_website_and_page(JSONB) TO authenticated, service_role;
