import type { Runner, SuiteResult, TestResult } from '../core/types.js'
...
/**
 * Vue Composition-style plugin for ACT metadata enrichment
 * Usage: runner.use(useACTMetadata(rules))
 */
export function useACTMetadata(rules: Record<string, ACTRuleMetadata> = {}) {
  return function(runner: Runner): void {
    const metadataMap = new Map<string, ACTRuleMetadata>()

    // Register initial rules
    for (const [testName, metadata] of Object.entries(rules)) {
      metadataMap.set(testName, metadata)
    }

    // Hook into the run method to enrich results
    const originalRun = runner.run
    runner.run = async function(): Promise<SuiteResult[]> {
      const results = await originalRun.call(this)

      // Enrich test results with ACT metadata
      for (const suiteResult of results) {
        for (const testResult of suiteResult.tests) {
          const metadata = metadataMap.get(testResult.name)
          if (metadata) {
    ...
    // Add helper methods to the runner for registering rules
    ;(runner as any).registerACTRule = (testName: string, metadata: ACTRuleMetadata) => {
      metadataMap.set(testName, metadata)
    }

    ;(runner as any).registerACTRules = (newRules: Record<string, ACTRuleMetadata>) => {
      for (const [testName, metadata] of Object.entries(newRules)) {
        metadataMap.set(testName, metadata)
      }
    }
  }
